            ...
            <template v-else-if="successPaymentMethod === 'eft'">
              <div class="success-panel success-panel--eft pa-6 rounded-xl mb-8">
                <div class="d-flex align-start">
                  ...
                  <div class="flex-grow-1">
                    <div class="success-panel__title font-weight-bold mb-2">Pay by Manual EFT</div>
      ...
      <v-row v-else :justify="cart.lines.length ? 'start' : 'center'">
        <template v-if="cart.lines.length">
          <v-col cols="12" lg="7">
            ...
            <section class="checkout-panel checkout-panel--payment" aria-labelledby="payment-heading">
              ...
              <v-alert
                v-if="!shopAcceptCash && !shopAcceptEft && !shopAcceptPeach"
                type="warning"
                dense
                outlined
              ...
              <v-alert
                v-else-if="shopAcceptEft && !hasBankingDetails"
                type="info"
                dense
                outlined
                class="mb-4 rounded-lg"
              >
                <strong>Manual EFT unavailable:</strong> this shop has not published bank details yet.
                <template v-if="shopAcceptCash || (shopAcceptPeach && peachConfigured)">
                  Use another payment option or contact the store.
                </template>
              </v-alert>
              <v-alert
              ...
              <div class="payment-cards">
                ...
                >
                  ...
                  <div class="payment-card__content">
                    <div class="payment-card__title">Manual EFT</div>
                    <p class="payment-card__text mb-0">
                      Pay from your banking app, then upload proof. We confirm automatically when the reference matches,
                      otherwise the store reviews it.
                    </p>
                  </div>
                </button>
                <v-card
                  ...
                  outlined
                  class="mt-2 mb-2 pa-4 rounded-lg eft-bank-card"
                >
                ...
                >
                  ...
                  <div class="payment-card__icon-wrap payment-card__icon-wrap--peach">
        ...
        <v-col cols="12" lg="5">
          <v-card class="pa-6 rounded-xl sticky-summary" elevation="2">
            ...
            <div v-if="paymentMethod === 'cash_store'" class="text-body-2">
              <v-icon small class="mr-1">payments</v-icon>
              Cash when you get your order
            </div>
            <div v-else-if="paymentMethod === 'eft' && hasBankingDetails" class="text-body-2">
              <v-icon small color="primary" class="mr-1">account_balance</v-icon>
              Manual EFT to <strong>{{ bankName }}</strong>
            </div>
            <div v-else-if="paymentMethod === 'eft'" class="text-caption text--secondary">
              Manual EFT selected — banking details missing; choose another option or contact the store.
            </div>
...
export default {
  ...
  computed: {
    ...
    hasBankingDetails() {
      return Boolean(
        String(this.bankName || '').trim() &&
          String(this.bankAccountHolder || '').trim() &&
  ...
  async created() {
    ...
    if (enabled === 1) {
      if (this.shopAcceptCash) this.paymentMethod = 'cash_store'
      else if (eftLive) this.paymentMethod = 'eft'
      else this.paymentMethod = 'peach'
    }
  },
  methods: {
    formatZar,
    ...
    choosePayment(m) {
      ...
      if (m === 'eft' && !this.shopAcceptEft) {
        this.paymentHint = 'This shop does not accept Manual EFT.'
        return
      }
    ...
    async submitOrderEftProof() {
      ...
      try {
        ...
        if (auto) {
          ...
          this.successNeedsEftProof = false
    ...
    async submit(options = {}) {
      ...
      if (this.paymentMethod === 'eft' && !this.shopAcceptEft) {
        this.submitError = 'This shop does not accept Manual EFT.'
        return
      }
