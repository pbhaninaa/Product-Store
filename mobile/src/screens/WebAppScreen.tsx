...
import { PRODUCT_STORE_PRIMARY } from '../config/branding';
...
import { merchantSlugFromPath, shouldShowBottomNav } from '../navigation/navigationMode';
...
export default function WebAppScreen() {
  ...
  const [webPath, setWebPath] = useState(CONFIG.INITIAL_WEB_PATH);
  const [userRole, setUserRole] = useState<string | null>(null);
  const [loggedIn, setLoggedIn] = useState(false);
  const [merchantSlug, setMerchantSlug] = useState(CONFIG.DEFAULT_MERCHANT_SLUG);
  ...
  const initialUri = `${CONFIG.WEB_APP_URL.replace(/\/+$/, '')}${CONFIG.INITIAL_WEB_PATH}`;

  const tabs = useMemo(() => tabsForRole(userRole, merchantSlug), [userRole, merchantSlug]);
    ...
    (event: { nativeEvent: { data: string } }) => {
      ...
      if (isWebAppStateMessage(parsed)) {
        ...
        if (parsed.merchantSlug) {
          setMerchantSlug(parsed.merchantSlug);
        } else {
          setMerchantSlug(merchantSlugFromPath(parsed.path, CONFIG.DEFAULT_MERCHANT_SLUG));
        }
